...
var tml = {
  ...
  init: function(token, options, callback) {
    ...
    if (options.proxy || options.immediate) {
  ...
  initApplication: function(token, options, callback) {
    ...
    if (options.current_locale) {
      ...
      if (current_locale) {
        ...
        utils.setCookie(cookie_name, utils.encode(cookie));
      }
  ...
  translateNow: function(){
    ...
    var FE = function(arr, fn) {
      for (var n = arr.length - 1; n > -1; n--) fn(arr[n], n);
    }

    var MO = function(mutations) {
      var nodeList = [];
      if (mutations.length > 0) {
        mutations.forEach(function(mutation) {
          ...
          if (nodes.length > 0) {
            FE(nodes, function(node) {
              if(node.tagName && node.tagName.toLowerCase().indexOf("tml:") != -1) return;
              if(node.tagName && node.tagName.toLowerCase().indexOf("script") != -1) return;
    ...
    observer = new MutationObserver(MO);
